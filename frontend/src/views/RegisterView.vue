...
<template>
  <div class="flex min-h-screen items-center justify-center bg-gray-100">
    <div class="w-full max-w-md bg-white rounded-lg shadow-md p-8">
      <h2 class="text-2xl font-bold text-center mb-6">Crea Account</h2>
      <div v-if="message" :class="`p-3 rounded mb-4 text-sm ${isError ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`">
        {{ message }}
      </div>
      <form @submit.prevent="handleRegister" class="space-y-4">
        <input v-model="form.displayName" placeholder="Nome Visualizzato (es. Mario Rossi)" required class="w-full border p-2 rounded">
      ...
      <p class="mt-4 text-center text-sm">
        Hai già un account? <router-link to="/login" class="text-indigo-600">Accedi</router-link>
      </p>
    </div>
  </div>
</template>